...
- **2026-08-26 — [Los artifacts de Claude no sirven para público externo]:** Un artifact publicado en claude.ai exige sesión iniciada para abrirse; el navegador devuelve 401/403 a quien no tiene cuenta. Para audiencias externas y no técnicas se sirve la misma página por GitHub Pages sobre el repo público (`gh api -X POST repos/OWNER/REPO/pages -f "source[branch]=main" -f "source[path]=/"`). **Por qué importa:** el caso trampa es una página cuyo primer paso es "crea tu cuenta de Claude", porque nadie puede leerla sin tener ya lo que la página pide. Antes de mandar cualquier enlace a alguien de fuera, comprobarlo con `curl -o /dev/null -w '%{http_code}'` desde una sesión sin autenticar.

- **2026-08-26 — [Outlook: lectura sí, escritura no]:** El conector MS365 del tenant de CG Constructora lee calendario y correo, pero `outlook_create_draft` falla con 403 `ErrorAccessDenied` porque `Mail.ReadWrite` no está consentido por el administrador. **Por qué importa:** no prometer borradores ni envíos automáticos de correo en flujos de este tenant. Entregar el cuerpo del mensaje como archivo HTML para copiar y pegar, y la lista de destinatarios aparte.
